from flask import json, jsonify, url_for, abort
...
def create_user():
    ...
    if User.query.filter_by(username=data["username"]).first():
        return bad_request(
            "please use a different username, this is already taken"
        )
    if User.query.filter_by(email=data["email"]).first():
        return bad_request(
            "please use a different email address, this is already taken"
        )
    user = User()
